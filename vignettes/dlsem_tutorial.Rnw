...
The main idea behind structural equation modelling (SEM, \cite{kline00})
is to perform a quantitative assessment
of the dependence relationships among the considered variables,
which is subsequently employed to perform path analysis, that is the
decomposition of the causal effect of any variable on another.

Here, an extension of SEM, called Distributed-lag structural equation modelling (DLSEM),
is proposed to account for temporal
delays in the dependence relationships among the variables.
This topic is of great interest in econometric problems,
as problem domains are typically dynamic and traditional (static)
path analysis cannot be applied.
However, there is no consensus on the formal specification
of lag relationships to be applied,
as a given application typically requires to take into
account specific domain knowledge.
%
%there is no widely accepted methodology so far
%due to two main issues: problema dell'attribuzione (in senso causale) e della stima del lag.
%
%DLSEM can be employed to perform path analysis, that is the
%decomposition of the causal effect of any variable on another,
%at different time lags.
%
DLSEM consists in enhancing linear regression models with
second order polynomial lag shapes \cite[Chapter 6]{baltagi08}.
One one hand, second-order polynomial lag shapes are simple to be estimated,
and, on the other hand, the interpretation of parameters is clear for
...
In this vignette, theory on distributed-lag structural equation
modelling is presented in Section \ref{sec:theory}, then the practical
use of \texttt{dlsem} is illustrated through a worked example in
...
%and distributed-lag linear regression \cite[Chapter 6]{baltagi08}.
...
The overall causal effect of $V_3$ on $V_6$ is the sum
...
See \cite[Chapter 6]{baltagi08} for further details
...
and consumer surplus independently from productivity \cite{renkow11,walker08}.
Here, a distributed-lag structural
equation model with DAG shown in Figure \ref{fig:dag0} is
estimated from dataset $\mathsf{agres}$ in order to test
whether the influence through time of
...
Dataset $\mathsf{agres}$
contains data for $10$ European countries (Austria, Germany, Spain, Finland,
...
%
In this illustration, we choose to
...
In this illustration, we indicate the country as the group factor,
...
We see that all edges are statistically significant,
excepting the one linking research activity to profitability.
...
Here we perform two path analysis tasks: one from research activity to profitability
and the other from research activity to consumer surplus.
For both, we focus on time lags 5, 10, 15, 20 and 25, and request
...
Our proposal appears as a suitable solution, because
second-order polynomial lag shapes involve simplicity of estimation
and a clear interpretation of parameters for domain experts,
so that prior knowledge can be taken into account in statistical analyses
by introducing simple mathematical constraints.
In our empirical analysis, we forced each lag shape to have
a maximum gestation lag of $3$ years and minimum lag width of $5$ years.

The empirical analysis here presented can be extended by considering research
investment as a direct cause of research activity, as well as a larger
number variables to better measure the macroeconomic state of the system.
%Although the two proposed lag shapes can represent, in principle,
%a large number of real-world events, future work may be directed towards
%the development of lag shapes with further special features.
