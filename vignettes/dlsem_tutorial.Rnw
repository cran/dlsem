...
%\VignetteKeyword{Impact analysis}
...
\date{\texttt{dlsem} version 1.7 -- 10 January 2017}
...
for structural equation modelling with constrained lag shapes (DLSEM: \citealp{magrini16}).
DLSEM is suited to perform dynamic causal inference, that is to
assess and disentangle causal effects at different time lags.
The vignette is structured as follows.
In Section \ref{sec:theory}, theory on structural equation modelling
with constrained lag shapes is presented.
In Section \ref{sec:install}, instructions for the installation
of the \texttt{dlsem} packages are provided.
...
In the Pearl's framework \citep{pearl00}, structural equation modelling (SEM)
consists of applying a linear regression model to each variable, and
all linear regression models define an acyclic directed graph (DAG).
In the DAG, variables are represented by nodes, a node
...
Also, the DAG has a causal interpretation, and a causal effect is associated
to each edge, directed path or couple of nodes to represent expected
changes induced by an intervention (\citealp[Chapter 5.3]{pearl00}; \citealp{pearl12}):
\begin{itemize}
...
\end{itemize}
Thus, SEM can be employed to assess and decompose the average change
in the value of any variable induced by an intervention provoking a
unit variation in the value of any other variable.
...
Distributed-lag structural equation modelling (DLSEM) is
an extension of SEM, where a constrained lag shape
is applied to each covariate in each regression model.
For DLSEM, the DAG does not explicitly include time lags, and
...
DLSEM can be employed to assess and decompose the causal effect of any variable
...
$\mathsf{R}$ version 2.1.0 or higher, which is freely available at
\url{http://www.r-project.org/}.

To install the \texttt{dlsem} package, type the following
in the $\mathsf{R}$ command prompt:
...
and \texttt{R} will automatically install the package to
...
The latest version of \texttt{dlsem} is 1.7.
...
%of the covariance matrix of estimates \citep{newey78} is applied.
...
$\mathsf{max.gestation}$, $\mathsf{max.lead}$, $\mathsf{min.width}$ and $\mathsf{sign}$.
...
including, respectively, the maximum gestation lag, the maximum lead lag,
the minimum lag width and the sign (either '+' for non-negative, or '-'
...
exogenous variables.
Also, we request the logarithmic transformation and provide
control options to argument $\mathsf{control}$:
...
\subsection{Assessment and decomposition of causal effects}
\label{sub:example4}
...
The Theory and Practice of Econometrics. John Wiley \& Sons, 2nd ed., New York, US-NY.

\bibitem[Lauritzen(1996)]{lauritzen96} S. L. Lauritzen (1996). Graphical Models.
...
%\bibitem[Newey and West(1978)]{newey78} W. K. Newey, and K. D. West (1978).
