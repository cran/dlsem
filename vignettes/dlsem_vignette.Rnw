...
%\VignetteKeyword{Constrained lag shape}
%\VignetteKeyword{Directed acyclic graph}
...
\date{\texttt{dlsem} version 1.8 -- 13 June 2017}
...
with constrained lag shapes (DLSEMs: \citealp{magrini16}).
Endpoint-constrained quadratic, quadratic decreasing and gamma
lag shapes are available.
DLSEMs allow to perform dynamic causal inference, that is to
...
structural equation models with constrained lag shapes is presented.
...
with the contribution of \citet{wright34} and further elaborated
by \citet{pearl00} in the context of causal inference.
...
intervention (\citealp[Chapter 5.3]{pearl00}; \citealp{pearl12}).
For a linear SEM:
\begin{itemize}
...
\end{itemize}
Thus, a linear SEM can be employed to assess and decompose the average change
...
a SEM where each regression model is a distributed-lag
linear regression model with constrained lag shapes.
...
A DLSEM can be exploited to assess and decompose the causal effect of any variable
...
The latest version of \texttt{dlsem} is 1.8.
...
is the definition of the model code, which is meant to include the
formal specification of the regression models in the DLSEM.
%
