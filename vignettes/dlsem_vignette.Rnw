...
\date{\pkg{dlsem} version 2.2 -- 05 March 2018}
...
and $\epsilon_t$ is the random error at time $t$ uncorrelated with the covariates and with $\epsilon_{t-1}$.
...
and $\mathcal{L}_m$ be the set of all the possible ordered $m$-uples
...
e(<V_{d_0},\ldots,V_{d_m}>;d_0=i,d_m=j)=\sum_{(l_1,\ldots,l_m)\in\mathcal{L}_m}\hspace{0.3cm}\prod_{k=1}^m b^{(l_k)}_{d_k|d_{k-1}}
\end{equation}
...
is obtained by summing all the instantaneous causal effects at each time lag up to $l$.
%
...
The latest version of \pkg{dlsem} is 2.2.
...
\caption{Pathwise causal lag shapes (upper panels) and the overall one (lower panel)
connecting the number of job positions to the amount of greenhouse gas emissions.
