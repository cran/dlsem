...
\date{\texttt{dlsem} version 1.4 -- \today.}
...
where each regression model is enhanced by second-order polynomial lag shapes,
...
with second-order polynomial lag shapes is provided
in Section \ref{sec:theory}, then
the practical use of \texttt{dlsem} is illustrated in Section \ref{sec:example}
through an empi\-rical analysis on EUROSTAT data aiming at assessing
the impact of technological innovation on European Agriculture.
...
to each edge, directed path or couple of variables
(tracing rules: \cite{wright34}; see also \cite{pearl12}):
\begin{itemize}
...
each regression model is enhanced by second-order polynomial
...
through a simplified impact assessment problem
inspired by the empirical analysis in \cite{bartolini16},
aiming at testing
whether the influence through time of the registration of
...
is the specification of the model code containing
the DAG addressing the research question, together with assumptions
