...
\date{\pkg{dlsem} version 2.3 -- 18 May 2018}
...
Section \ref{sec:remarks} includes final remarks and considerations
on future development of the package.
...
\beta_{j,l} = \begin{cases}
  \phi_0 & l=0\\
  \sum_{q=0}^Q \phi_q l^q & \text{otherwise}
  \end{cases}
\end{equation}
For instance, for $q=2$ we have that $\beta_{j,l} =\phi_0+\phi_1 l+\phi_2 l^2$.
...
Constrained lag shapes \citep[Chapters 9-10]{judge85} overcome this issue.
...
but the corresponding values of $a_j$ and $b_j$ may be computed
through numerical approximation from the values of $\delta_j$ and $\lambda_j$.
%
...
a trend in order to obtain unbiased estimates with ordinary least squares \citep{granger74}.
...
%\paragraph{Linear Markovian structural causal models}
%
...
In a linear Markovian SCM, the computation of causal effects involves
the coefficients of the regression models only, without the need of Formula \ref{eq:markov_ce}.
...
\end{equation}
where $b_{k|k-1}$ is the coefficient of $V_{k-1}$ in the regression model of $V_k$.
...
$<V_1,V_2,V_4>$ with pathwise causal effect $b_{2|1}\cdot b_{4|2}$,
and $<V_1,V_3,V_4>$ with pathwise causal effect $b_{3|1}\cdot b_{4|3}$.
Thus, the overall causal effect of $V_1$ on $V_4$ is equal to
%$\text{E}(V_4 \mid \text{do}(\Delta V_1=1))$
$b_{2|1}\cdot b_{4|2}+b_{3|1}\cdot b_{4|3}$.
...
e(<V_{d_0},\ldots,V_{d_m}>;d_0=i,d_m=j)=\sum_{(q_1,\ldots,q_m)\in Q_m^{(l)}}\hspace{0.3cm}\prod_{k=1}^m b^{(q_k)}_{d_k|d_{k-1}}
\end{equation}
where $b^{(l_k)}_{d_k|d_{k-1}}$ is the coefficient of $V_{d_{k-1}}$ at lag $l_k$
...
\end{itemize}
%The causal effects just defined are denoted as \textit{instantaneous}
%causal effects, as they are evaluated at a single time lag.
%The \textit{cumulative} causal effect at a pre-specified time lag, say $l$,
%is obtained by summing all the %instantaneous
%causal effects at each time lag up to $l$.
%
...
The latest version of \pkg{dlsem} is 2.3.
...
%
}, and operators \code{quec.lag}$(\cdot)$, \code{qdec.lag}$(\cdot)$ and \code{gamma.lag}$(\cdot)$
...
Operator \code{gamma.lag}$(\cdot)$ has three arguments:
the name of the covariate to which the lag shape is applied,
...
%Also, the user may choose whether the instantaneous 
...
%Nevertheless, additional lag shapes with further specific features may be added in the future.
...
%for most real-world applications, a heuristic search based on forward
%model selection is currently implemented.
%Further development of the package may be directed towards the
...
Important functionalities like random effects estimation and autoregressive lag shapes
will be implemented in the next releases.
Also, additional lag shapes with further specific features may be added in the future.
