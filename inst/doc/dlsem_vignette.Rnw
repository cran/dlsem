...
\title{Distributed-lag linear structural equation models in \texttt{R}:\\the \texttt{dlsem} package}
\author{Alessandro Magrini\\
Dep. Statistics, Computer Science, Applications\\
University of Florence, Italy\\
<magrini@disia.unifi.it>}
\date{\texttt{dlsem} version 2.0 -- 05 December 2017}
...
Recently, \cite{magrini16} proposed an extension of linear
SCMs composed of distributed-lag linear regression models \citep[Chapters 9-10]{judge85},
called \textit{distributed-lag linear structural equation model} (DLSEM).
A DLSEM accounts for temporal delays in the dependence
relationships among the variables and allows to
assess causal effects at different time lags.

Package \texttt{dlsem} implements inference functionalities for DLSEMs
...
represents its regression coefficient at different time lags.
...
coefficient, but values $a_j$ and $b_j$ can be computed through numerical approximation.
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.4\textwidth]{figures/sampledag.png}
\caption{The directed acyclic graph of a structural causal model.
...
an extension of linear SCMs, where each factor of the probability
...
Before installing \texttt{dlsem}, you must have installed \texttt{R} version 2.1.0
or higher, which is freely available at \url{http://www.r-project.org/}.

To install the \texttt{dlsem} package, type the following in the \texttt{R} command prompt:
...
and \texttt{R} will automatically install the package to your system from CRAN.
In order to keep your copy of \texttt{dlsem} up to date, use the command:
...
The latest version of \texttt{dlsem} is 2.0.
...
that variable is $0$ for time lags greater than $0$ (no lag shape).
...
After the estimation, the user can display the DAG where each edge
is coloured with respect to the sign of the estimated causal effect
(green: positive, red: negative, light gray: not significant).
...
All causal effects result statistically significant,
...
The main arguments are the name of one or more variables generating the causal effects
(argument \code{from}), and the name of the variable receiving the causal effects
(argument \code{to}).
...
The group factor and exogenous variables cannot be indicated as starting
or ending variables, and they are not involved in the computation of causal effects,
but the resulting causal effects are net of their influence.
...
are shown in the component named \code{overall}.

Since the logarithmic transformation was applied to all quantitative
variables, causal effects above are interpreted as elasticities,
that is, for a $1\%$ of job positions more,
greenhouse gas emissions are expected to grow by $0.61\%$ after $5$ years
and by $1.10\%$ after $10$ years.
The influence ends before $15$ years, as the cumulative causal
...
\begin{figure}[!h]
\centering
{\includegraphics[width=0.48\textwidth]{figures/lagsh1.pdf}}\quad
{\includegraphics[width=0.48\textwidth]{figures/lagsh2.pdf}}\\
{\includegraphics[width=0.48\textwidth]{figures/lagsh3.pdf}}
