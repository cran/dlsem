...
\date{\pkg{dlsem} version 2.4.3 -- 20 August 2019}
...
The latest version of \pkg{dlsem} is 2.4.3.
...
to compare them according to the Bayesian Information Criterion:
...
The model with endpoint-constrained quadratic lag shapes has
the lower BIC and thus the best fit to data.
...
Further functionalities for linear models will be implemented in the next releases.
